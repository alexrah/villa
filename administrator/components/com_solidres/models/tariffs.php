...
class SolidresModelTariffs extends JModelList
{
    ...
    protected function getListQuery()
    {
		...
		$numberOfNights = $this->getState('filter.number_of_nights', 0);
		if ($numberOfNights > 0)
		{
			$query->where('
			CASE
				WHEN t.d_min > 0 AND t.d_max > 0 THEN t.d_min <= '.(int)$numberOfNights.' AND t.d_max >= '.(int)$numberOfNights .'
				WHEN t.d_min = 0 AND t.d_max > 0 THEN t.d_max >= '.(int)$numberOfNights .'
				WHEN t.d_min > 0 AND t.d_max = 0 THEN t.d_min <= '.(int)$numberOfNights .'
