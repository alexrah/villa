...
class SolidresTableRoomType extends JTable
{
	...
	public function delete($pk = null)
	{
		...
		$query->where('tariff_id IN (SELECT id FROM '.$this->_db->quoteName('#__sr_tariffs').' WHERE room_type_id = ' . $this->_db->quote($pk) .')');
		$this->_db->setQuery($query)->execute();
		$query->clear();
		$query->delete($this->_db->quoteName('#__sr_tariffs'))->where('room_type_id = '. $this->_db->quote($pk));
		$this->_db->setQuery($query)->execute();
