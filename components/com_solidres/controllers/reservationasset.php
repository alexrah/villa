...
class SolidresControllerReservationAsset extends JControllerLegacy
{
	...
	public function __construct($config = array())
	{
		...
		{
			$this->reservationAssetId = $this->input->getUint('raid'); // compatible with room quantity selection
		}
		...
		$this->app->setUserState($this->context.'.deposit_required', $reservationAssetTable->deposit_required);
		$this->app->setUserState($this->context.'.deposit_is_percentage', $reservationAssetTable->deposit_is_percentage);
		$this->app->setUserState($this->context.'.deposit_amount', $reservationAssetTable->deposit_amount);
		$this->app->setUserState($this->context.'.tax_id', $reservationAssetTable->tax_id);
	}
	...
	public function checkavailability()
	{
		...
		try
		{
			$isCheckInCheckOutValid = $solidresReservation->isCheckInCheckOutValid($checkIn, $checkOut, $conditions);
		}
		catch (Exception $e)
		{
			switch ($e->getCode())
	...
	public function getRoomTypeForm()
	{
		...
		$form = new JLayoutFile('asset.roomtypeform');
	...
	public function getCheckInOutForm()
	{
		...
		$form = new JLayoutFile('asset.checkinoutform');
		$displayData = array(
			'tariff' => $tariff,
			'assetId' => $assetId,
			'roomTypeId' => $roomtypeId,
	...
	public function startOver()
	{
		...
		$this->setRedirect(JRoute::_('index.php?option=com_solidres&view=reservationasset&id='.$id.'#form', false));
	}
}