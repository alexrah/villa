...
if (!$isStandardTariff) :
	$dateCheckIn = JDate::getInstance($tariff->valid_from, $timezone)->add(new DateInterval('P' . $minDaysBookInAdvance . 'D'));
	$dateCheckOut = JDate::getInstance($tariff->valid_from, $timezone);
else :
	$dateCheckIn = JDate::getInstance('now', $timezone);
...
<div class="inner">
	...
	<?php // this field must always be "Y-m-d" as it is used internally only ?>
	<input type="hidden" name="checkout" value="<?php echo $defaultMinCheckOutDateString = $defaultMinCheckOutDate->format('Y-m-d', true); ?>" />

	<input type="hidden" name="fts" value="<?php echo time() ?>" />

	<button class="btn btn-block btn-primary primary searchbtn" data-roomtypeid="<?php echo $displayData['roomTypeId'] ?>" data-tariffid="<?php echo $tariff->id ?>" type="button"><i class="icon-search uk-icon-search fa-search "></i> <?php echo JText::_('SR_SEARCH')?></button>
